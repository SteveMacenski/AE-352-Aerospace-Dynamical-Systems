...
F = [robot.sc.m*eye(3,3) zeros(3,3) zeros(3,1) zeros(3,1) R_3in0 zeros(3,2) R_3in0 zeros(3,2);
     robot.rw4.m*eye(3,3) -robot.rw4.m*R_3in0*wedge(robot.o_4in3) zeros(3,1) zeros(3,1) -R_3in0 zeros(3,2) zeros(3,3) zeros(3,2);
     robot.rw5.m*eye(3,3) -robot.rw5.m*R_3in0*wedge(robot.o_5in3) zeros(3,1) zeros(3,1) zeros(3,3) zeros(3,2) -R_3in0 zeros(3,2);
     zeros(3,3) robot.sc.J_in3 zeros(3,1) zeros(3,1) wedge(robot.rw4.ppin_in3) S4 wedge(robot.rw5.ppin_in3) S5;
     zeros(3,3) robot.rw4.J_in4*R_4in3' robot.rw4.J_in4*t4 zeros(3,1) -wedge(robot.rw4.ppin_in4)*R_4in3' -R_4in3'*S4 zeros(3,3) zeros(3,2);
     zeros(3,3) robot.rw5.J_in5*R_5in3' zeros(3,1) robot.rw5.J_in5*t5 zeros(3,3) zeros(3,2) -wedge(robot.rw5.ppin_in5)*R_5in3' -R_5in3'*S5];

h = [zeros(3,1);
     -robot.rw4.m*R_3in0*wedge(w_03in3)*wedge(w_03in3)*robot.o_4in3;
     -robot.rw5.m*R_3in0*wedge(w_03in3)*wedge(w_03in3)*robot.o_5in3;
     -wedge(w_03in3)*robot.sc.J_in3*w_03in3-t4*u4-t5*u5;
     -robot.rw4.J_in4*(R_4in3*wedge(w_34in4))'*w_03in3+R_4in3'*t4*u4-wedge(w_04in4)*robot.rw4.J_in4*w_04in4;
     -robot.rw5.J_in5*(R_5in3*wedge(w_35in5))'*w_03in3+R_5in3'*t5*u5-wedge(w_05in5)*robot.rw5.J_in5*w_05in5];
...
R_3in0 = RX(theta(1))*RY(theta(2))*RZ(theta(3));
